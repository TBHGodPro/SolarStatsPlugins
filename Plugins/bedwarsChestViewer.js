...
player.proxy.on("incoming", (data, meta) => {
  if (meta.name == "open_window") {
    if (JSON.parse(data.windowTitle).translate == "container.enderchest")
      ECwindowId = data.windowId;
    if (JSON.parse(data.windowTitle).translate == "container.chest")
      TCwindowId = data.windowId;
  }
  if (
    meta.name == "window_items" &&
    (ECwindowId == data.windowId || TCwindowId == data.windowId)
  ) {
    ...
    if (ECwindowId == data.windowId || TCwindowId == data.windowId) {
      ...
      if (ECwindowId == data.windowId) {
        if (data.slot > 26 && data.mode == 1) {
          heldItem = data.item;
          data.slot = ECitems.indexOf(
            ECitems.find(
              (i) =>
                i.blockId == data.item.blockId &&
                i.itemDamage == data.item.itemDamage &&
                i.nbtData == data.item.nbtData &&
                i.itemCount < 64
            )
          );
          if (data.slot == -1 || data.slot > 26)
            data.slot = ECitems.indexOf(ECitems.find((i) => i.blockId == -1));
        ...
        if (data.slot > 26 && data.mode == 1) {
          ...
          if (data.slot == -1 || data.slot > 26)
            data.slot = TCitems.indexOf(TCitems.find((i) => i.blockId == -1));
          if (data.slot > 26) return;
        } else if (data.mode == 1) {
          TCitems[data.slot] = {
            blockId: -1,
...
ECCommand.onTriggered = async () => {
  if (ECitems?.length) {
    const inventory = new Inventory(
      ...
      "§5§lEnder Chest §r§f(At Last View)",
      27
    );
    ...
    for (var i of ECitems) {
      ...
      if (i?.nbtData?.value?.display?.value?.Name?.value)
        item.displayName = i?.nbtData?.value?.display?.value?.Name?.value;
      if (i?.nbtData?.value?.display?.value?.Lore?.value?.value)
        item.lore = i?.nbtData?.value?.display?.value?.Lore?.value?.value;
...
TCCommand.onTriggered = async () => {
  if (TCitems?.length) {
    const inventory = new Inventory(
      InventoryType.CONTAINER,
      "§a§lTeam Chest §r§f(At Last View)",
      27
    );
...
registerPlugin({
  ...
  description:
    "View Your Ender Chest And Team Chest From Anywhere | `/ec` `/tc`",
