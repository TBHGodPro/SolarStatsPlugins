require("../updater").default("gameStats");

const { PlayerModule, Item, getConfigSync, getConfig } = toolbox;

const settingItem = new Item(4);
settingItem.displayName = "§rGame Stats";
settingItem.lore = ["", "§7Shows Your Opponent's Game Stats", "§7In Duels Once They Join", "", `§7Current: §${getConfigSync().modules.gameStats ? "aEnabled" : "cDisabled"}`];

const module = new PlayerModule("LC Staff Mods (X-RAY)", "Enable Staff Mods For Lunar Client", settingItem, "gameStats");

var gameMode = null;

async function list(id, name) {
	await new Promise(res => setTimeout(res, 1500));
	if (!(await getConfig()).modules.gameStats || name == player.client.username || !gameMode) return;
	const data = await fetch(`https://api.hypixel.net/player?key=${encodeURIComponent((await getConfig()).apiKey)}&uuid=${encodeURIComponent(id)}`);
...
module.handle("LocationUpdate", async status => {
	if (status.mode?.startsWith("DUELS") && (await getConfig()).modules.gameStats) {
		game(status);
	} else {
		lobby();
	}
...
registerPlugin({
	name: "Game Stats",
	description: "Shows Your Opponent's Game Stats In Duels Once They Join",
	version: "1.0.0",
	author: "TBHGodPro"
});
